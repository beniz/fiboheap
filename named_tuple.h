		...
		std::make_pair(																			\
			[](auto wrapped_base, auto index) {												\
				...
				struct accessor : public base_t {											\
					using base_t::base_t;													\
					decltype(auto) name() { return std::get<index_t::value>(*this); }		\
					decltype(auto) name() const { return std::get<index_t::value>(*this); }	\
				};																			\
				return accessor{};															\
			},																				\
			expr)
...
namespace named_tuple {
	namespace detail {
		...
		struct named_tuple<Base, ignored> : public Base { using Base::Base; };

		template <typename Base, size_t index, typename Name, typename... Rest>
		struct named_tuple<Base, index, Name, Rest...> : decltype(
			std::declval<Name>()(
